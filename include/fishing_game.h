...
#define FISHING_BAR_BOUNCINESS          1.8  // The greater the number, the less the bar will bounce when it hits the left side. (Decimals are encouraged, as a little goes a long way.)
...
#define FISH_FIRST_MOVE_DELAY           1.0  // Number of seconds before the fish will make its first movement.
#define FISH_ICON_HITBOX_WIDTH          12   // Width of the fish's hitbox in number of pixels.
#define FISH_SPEED_MULTIPLIER           75   // Global fish speed multiplier. It is a percent, so 50 would be half speed and 200 would be double speed.

#define DEFAULT_TREASURE_CHANCE         45   // Percent chance a random treasure will spawn if FG_VAR_TREASURE_CHANCE is 0.
