...
static void TMCase_ItemPrintFunc(u8 windowId, u32 itemId, u8 y)
{
    if (itemId != -2)
    {
        if (GetItemSecondaryId(BagGetItemIdByPocketPosition(POCKET_TM_HM, itemId)) == TM)
        {
...
static void Task_SelectTMAction_FromFieldBag(u8 taskId)
{
    ...
    //show HM icon
    if (GetItemSecondaryId(gSpecialVar_ItemId) == HM)
    {
...
static void Task_SelectTMAction_Type3(u8 taskId)
{
    ...
    if (GetItemSecondaryId(BagGetItemIdByPocketPosition(POCKET_TM_HM, data[1])) == TM)
    {
